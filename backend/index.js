...
fastify.setErrorHandler((error, request, reply) => {
	...
	if (error.validation) {
		if (request.routerPath === '/api/players' && request.method === 'POST')
			return reply.status(400).send({ error: 'Invalid player data: name and password are required, no extra fields allowed.' });
			...
			return reply.status(400).send({ error: 'Invalid player info data: "name" and/or "e-mail" required.' });
		else
